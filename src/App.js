...
function App() {
  ...
  return (
    <Router>
      <div>
        <nav className='flex bg-[#222222] p-[8px] justify-between items-center h-[96px] max-w-full mx-auto px-4'>
          <ul className=''>
            <li>
              <label className='flex text-[#ff6600] text-2xl w-full font-extrabold p-[5px] m-0 cursor-pointer uppercase'><Link to="/">Character Profile</Link></label>
              <label className='flex min-w-fit text-white text-md font-semibold pl-[5px]'><Link to="/">Home</Link></label>
            </li>
          </ul>
        </nav>
        ...
        <Switch>
          <Route path="/" exact>
            ...
            {filteredCharacters.length > 0 ? (
              ...
              <p className='pl-6 pb-40 pt-40 items-center text-white'>No matching characters found.</p>
            )}
